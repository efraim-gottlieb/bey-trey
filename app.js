...
function displayPostStatistics() {
  const count = apiCallTimes.length;
  const minRunTime = Math.min(...apiCallTimes);
  const maxRunTime = Math.max(...apiCallTimes);
  const statistics = {
    count,
    minRunTime,
    maxRunTime,
...
function displayApiPerformance() {
  let acc = 0;
  for (let i of apiCallTimes) {
    acc += i;
  }
  console.log(`${acc / apiCallTimes.length}ms`);
}
